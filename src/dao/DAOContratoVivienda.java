...
public class DAOContratoVivienda {
	...
	public ContratoViviendaVO getContratoVivienda(Long id) throws SQLException, Exception {
		...
		String sql = String.format("SELECT * FROM CONTRATORECIDENCIA WHERE ID = '%2$s' ", RELACIONADO, id);
	 ...
	 * 
	 * @param contrato
	 *            ContratoViviendaVO que desea agregar a la Base de Datos
	 * @throws SQLException
	 *             SQLException Genera excepcion si hay error en la conexion o
	 *             en la consulta SQL
	 * @throws Exception
	...
	public void addContratoVivienda(ContratoViviendaVO contrato) throws SQLException, Exception {
		...
		String sql = String.format(
				"INSERT INTO CONTRATORECIDENCIA (ID,FECHAINICIO, FECHAFIN, CUARTO, RELACIONADO) VALUES (%2$s, '%3$s', '%4$s', '%5$s',%6$s)",
				RELACIONADO, contrato.getId(), contrato.getFechaI(), contrato.getFechaF(), contrato.getCuarto(),
				contrato.getUsuario());
	...
	/**
	 * Metodo que actualiza la informacion del contrato en la Base de Datos que
	 * tiene el identificador dado por parametro<br/>
	 * <b>Precondicion: </b> la conexion a sido inicializadoa <br/>
	 * 
	 * @param contrato
	 *            ContratoViviendaVO que desea actualizar a la Base de Datos
	 * @throws SQLException
	 *             SQLException Genera excepcion si hay error en la conexion o
	 *             en la consulta SQL
	 * @throws Exception
	...
	public void updateContratoVivienda(ContratoViviendaVO contrato) throws SQLException, Exception {
		...
		sql.append(String.format(
				"FECHAINICIO = '%1$s' AND FECHAFIN = '%2$s' AND CUARTO = '%3$s' AND RELACIONADO = '%4$s' ",
				contrato.getFechaI(), contrato.getFechaF(), contrato.getCuarto()));
	...
	public void deleteContratoVivienda(ContratoViviendaVO contrato) throws SQLException, Exception {
		...
		String sql = String.format("DELETE FROM CONTRATORECIDENCIA WHERE ID = %2$d", RELACIONADO, contrato.getId());
	 ...
	 * tabla CONTRATORECIDENCIA) en una instancia de la clase
	 * ContratoViviendaVO.
	 * 
	 * @param resultSet
	 *            ResultSet con la informacion de un contrato que se obtuvo de
	 *            la base de datos.
	 * @return ContratoViviendaVO cuyos atributos corresponden a los valores
