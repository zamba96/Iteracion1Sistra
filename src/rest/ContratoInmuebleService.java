...
public class ContratoInmuebleService {
	...
	public Response getContratosInmuebles() {

		try {
			TransactionManager tm = new TransactionManager(getPath());

			List<ContratoInmuebleVO> inmuebles;
			//Por simplicidad, solamente se obtienen los primeros 50 resultados de la consulta
			inmuebles = tm.getContratosInmuebles();
	 ...
	 * @return	<b>Response Status 200</b> - JSON ContratoInmuebleVO que contiene al contrato cuyo ID corresponda al parametro <br/>
	 * 			<b>Response Status 500</b> - Excepcion durante el transcurso de la transaccion
	 */
	@GET
			...
			@PathParam( "fechaF" ) String fechaF,@PathParam( "usuario" ) String usuario)
	{
		try{
			...
			ContratoInmuebleVO contrato = tm.getContratoInmueble(fechaI, fechaF, usuario);
	...
	@Produces({MediaType.APPLICATION_JSON})
	public Response addContratoInmueble(ContratoInmuebleVO contrato) {
		try {
			TransactionManager tm = new TransactionManager(getPath());
			tm.addContratoInmueble(contrato);
			return Response.status( 200 ).entity( contrato ).build( );			
		} catch (Exception e) {
			return Response.status( 500 ).entity( doErrorMessage( e ) ).build( );
		}
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({MediaType.APPLICATION_JSON})
	public Response updateBebedor(ContratoInmuebleVO contrato) {
		try {
			TransactionManager tm = new TransactionManager(getPath());
			//tm.updateContratoInmueble(contrato);
			return Response.status( 200 ).entity( contrato ).build( );
		} catch (Exception e) {
			return Response.status( 500 ).entity( doErrorMessage( e ) ).build( );
		}
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({MediaType.APPLICATION_JSON})
	public Response deleteBebedor(ContratoInmuebleVO contrato) {
		try {
			TransactionManager tm = new TransactionManager(getPath());
			tm.deleteContratoInmueble(contrato);
			return Response.status( 200 ).entity( contrato ).build( );
		} catch (Exception e) {
			return Response.status( 500 ).entity( doErrorMessage( e ) ).build( );
		}
