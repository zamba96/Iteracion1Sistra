...
public class RelMasivaService {
	// ----------------------------------------------------------------------------------------------------------------------------------
		...
		@Path("{id}")
		@Produces({ MediaType.APPLICATION_JSON })
		public Response getRelMasivaById(@PathParam("id") Long id) {
			try {
				TransactionManager tm = new TransactionManager(getPath());

				RelMasivaVO contrato = tm.getRelMasiva(id);
		...
		@Produces({ MediaType.APPLICATION_JSON })
		public Response addRelMasiva(RelMasivaVO contrato) {
			try {
				TransactionManager tm = new TransactionManager(getPath());
				tm.addRelMasiva(contrato);
				return Response.status(200).entity(contrato).build();
			} catch (Exception e) {
				return Response.status(500).entity(doErrorMessage(e)).build();
			}
		...
		@Consumes({ MediaType.APPLICATION_JSON })
		@Produces({ MediaType.APPLICATION_JSON })
		public Response updateBebedor(RelMasivaVO contrato) {
			try {
				TransactionManager tm = new TransactionManager(getPath());
		...
		@Consumes({ MediaType.APPLICATION_JSON })
		@Produces({ MediaType.APPLICATION_JSON })
		public Response deleteRelMasiva(RelMasivaVO contrato) {
			try {
				TransactionManager tm = new TransactionManager(getPath());
				tm.deleteRelMasiva(contrato);
				return Response.status(200).entity(contrato).build();
			} catch (Exception e) {
				return Response.status(500).entity(doErrorMessage(e)).build();
			}
