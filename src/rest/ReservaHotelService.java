...
public class ReservaHotelService {
	//----------------------------------------------------------------------------------------------------------------------------------
			 ...
			 * @return	<b>Response Status 200</b> - JSON ReservaHotelVO que contiene al reserva cuyo ID corresponda al parametro <br/>
			 * 			<b>Response Status 500</b> - Excepcion durante el transcurso de la transaccion
			 */
			@GET
					...
					@PathParam( "fechaF" ) String fechaF,@PathParam( "usuario" ) String usuario)
			{
				try{
					...
					ReservaHotelVO reserva = tm.getReservaHotel(fechaI, fechaF, usuario);
			...
			@Produces({MediaType.APPLICATION_JSON})
			public Response addReservaHostal(ReservaHotelVO reserva) {
				try {
					TransactionManager tm = new TransactionManager(getPath());
			...
			@Consumes({ MediaType.APPLICATION_JSON })
			@Produces({MediaType.APPLICATION_JSON})
			public Response updateBebedor(ReservaHotelVO reserva) {
				try {
					TransactionManager tm = new TransactionManager(getPath());
			...
			@Consumes({ MediaType.APPLICATION_JSON })
			@Produces({MediaType.APPLICATION_JSON})
			public Response deleteBebedor(ReservaHotelVO reserva) {
				try {
					TransactionManager tm = new TransactionManager(getPath());
